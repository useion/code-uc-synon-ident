...
module.exports = {
    ...
    insertSimilarwordsIntoWords: function (wordlist, mapping, wtype, type) {
        return new Promise(function (fulfill, reject) {
            ...
            var stmt = db.prepare("INSERT INTO Words(word,wtype,swmap,posmap,type) VALUES (?,?,?,?,?)");
            ...
            for (var i in wordlist) {
                if (wordlist.hasOwnProperty(i)) {
                    if (i == 0 && (wtype == 'n' || wtype == 'v' || wtype =='a')) {
                        ...
                        stmt.run(wordlist[i], wtype, mapping, wtype, type, function (err) {
                            if(err != null) {
                                console.log(err);
                                reject();
                            }
