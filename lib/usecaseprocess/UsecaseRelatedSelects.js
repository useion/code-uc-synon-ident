...
module.exports = {
    ...
    selectUseCaseIdAndName: function () {
        return new Promise(function (resolve, reject) {
            db.all("SELECT uid AS id,name,path FROM Use_cases", function (err, res) {
    ...
    selectSimWords: function(wid,builderArr){
        return new Promise(function(resolve,reject){
            ...
            db.all("SELECT word, swmap, type FROM Words WHERE swmap=?",[wid],function(err,res){
                if(err != null && res == null ){
                    console.log("select words data error "+ err);
                    reject(err);
                    return;
    ...
    countRelevantWordsInStep: function(sid){
        return new Promise(function(resolve,reject){
            ...
            db.all("select Words.wid, Words.word from Words "+
                " join Step_word on Step_word.wid=Words.wid "+
                " join Steps on Steps.sid=Step_word.sid "+
                " where (Steps.sid= ? )"+
                " AND Words.swmap = 0 "+
